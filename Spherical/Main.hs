...
main :: IO ()
main = navigate (build [1,2,3] [[4,5,6]]) -- [[4,5,6],[7,8,9]])
...
build (x:xs) ys = node
    ...
    (left, right) = horizontal xs up down (node, node)
...
horizontal :: [a] -> Quartic a -> Quartic a -> (Quartic a, Quartic a) -> (Quartic a, Quartic a)
horizontal set up down (origin, node)= case set of
    []     -> (,) node origin
    (x:xs) -> (,) last next
        where
        next          = Vertex x (link (edge up E) (edge down E) node right)
        (last, right) = horizontal xs (edge up E) (edge down E) (origin, next) 
...
vertical sets (origin, node) = case sets of
    ...
    ((x:xs):ys) -> (,) last next
        ...
        (left, right) = horizontal xs node down (origin, next)
